...
class User(UserMixin):
    ...
    def save(self):
        newUser = document.User(email=self.email, password=self.password, active=self.active)
        newUser.save()
    ...
    def get_by_email(self, email):
    	...
    	dbUser = document.User.objects.get(email=email)
    	if dbUser:
    ...
    def get_mongo_doc(self):
        if self.id:
            return document.User.objects.with_id(self.id)
        else:
            return None

    def get_by_id(self, id):
    	dbUser = document.User.objects.with_id(id)
    	if dbUser:
    		self.email = dbUser.email
    		self.active = dbUser.active
    		self.id = dbUser.id
